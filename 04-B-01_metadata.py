...
def process_single_movie_json(json_path, context):
    ...
    html_dir = os.path.join(BASE_DIR, f"data/webpages/html_{movie_slug}")
    ...
    script_log_path = os.path.join(movie_logs_dir, "03-A-2_jsonld_parser.json")
    ...
    for index, pub in enumerate(publishers, start=1):
        ...
        if not os.path.exists(html_file_path):
            ...
            if os.path.exists(alt_file_path):
                ...
                print("      [SKIP] HTML file not found on disk")
                summary_counts["Skipped (Webpage missing)"] += 1
